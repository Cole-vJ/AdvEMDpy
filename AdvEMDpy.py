...
if __name__ == "__main__":
    ...
    temp_time_series = np.sin(temp_time) + np.sin(5 * temp_time)

    temp_knot = np.linspace(0, 5 * np.pi, 41)

    emd_test = EMD(time=temp_time, time_series=temp_time_series)

    # edge effects:
    # 'neural_network', 'symmetric', 'symmetric_anchor', 'symmetric_discard', 'anti-symmetric',
    # 'characteristic_wave_Huang', 'characteristic_wave_Coughlin', 'slope_based_method',
    # 'improved_slope_based_method', 'average', 'none'

    edge_effects = 'symmetric'

    debug_bool = True

    method = 'b_spline'

    recursive_if_bool = False
    recursive_ht_bool = False
    dtht_bool = True

    dtht_method_temp = 'fft'

    matrix_bool = False

    intitial_bool = True

    # {'envelopes', 'inflection_points', 'binomial_average', 'enhanced'}

    dfts = 'envelopes'

    # {'median_filter', 'mean_filter', 'winsorize', 'winsorize_interpolate', 'HP', 'HW', 'downsample', 'none'}

    preprocess_temp = 'downsample'
    ...
    # 'sd', 'sd_11a', 'sd_11b', 'mft', 'edt', 'S_stoppage'

    stop = 'sd'
    stop_crit_new = 5

    storage = \
        emd_test.empirical_mode_decomposition(
            knot_time=temp_time, knots=temp_knot, smooth=True,
            smoothing_penalty=0.01, edge_effect=edge_effects, sym_alpha=0.1,
            stop_crit=stop, stop_crit_threshold=stop_crit_new,
            mft_theta_1=0.05, mft_theta_2=0.5, mft_alpha=0.05,
            mean_threshold=1, debug=debug_bool, verbose=True,
            spline_method=method, smooth_if=recursive_if_bool, smooth_ht=recursive_ht_bool,
            dtht=dtht_bool, dtht_method=dtht_method_temp, max_internal_iter=10, max_imfs=10, matrix=matrix_bool,
            initial_smoothing=intitial_bool, dft=dfts,
            order=15, increment=10, preprocess=preprocess_temp, preprocess_window_length=51,
            preprocess_quantile=0.9, preprocess_penalty=1, preprocess_order=13,
            preprocess_norm_1=2, preprocess_norm_2=1, ensemble=ensemble_bool,
            ensemble_sd=0.1, ensemble_iter=10, enhanced_iter=10,
            output_coefficients=output_coef, optimise_knots=op_knots,
            knot_method='bisection', output_knots=False,
            downsample_window='hamming', downsample_decimation_factor=20,
            downsample_window_factor=20, nn_m=200, nn_k=100, nn_method='grad_descent',
            nn_learning_rate=0.01, nn_iter=100)

    downsample_time = {}
    if preprocess_temp == 'downsample':
        downsample_time = np.linspace(0, 5 * np.pi, 51)

    for a in range(1, np.shape(storage[0])[0]):
        plt.figure(1)
        plt.plot(downsample_time, storage[0][a, :], label='Method')
        plt.legend(loc='lower left')
        plt.figure(2)
        plt.plot(downsample_time, storage[1][a, :], label='Method')
        plt.legend(loc='lower left')
        plt.figure(3)
        plt.plot(downsample_time[1:], storage[2][a, :], label='Method')
        plt.legend(loc='lower left')

    plt.show()

    emd_temp = EMD(time=temp_time, time_series=temp_time_series)
    outputs = emd_temp.empirical_mode_decomposition(knots=temp_knot, knot_time=temp_time)

    hilbert_outputs = hilbert_spectrum(time=temp_time, imf_storage=outputs[0], ht_storage=outputs[1],
                                       if_storage=outputs[2], max_frequency=10)

    plt.pcolormesh(hilbert_outputs[0], hilbert_outputs[1], hilbert_outputs[2], cmap='gist_rainbow',
                   vmin=np.min(hilbert_outputs[2]), vmax=np.max(hilbert_outputs[2]))
    plt.show()
